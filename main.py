...
MAX_WORKERS = 8  # ✅ زيادة من 5 لـ 8 لتحسين سرعة الدورة
...
# ✅ حذف MIN_SCORE_ALL_STRATEGIES و SIGNAL_LEVELS (dead code)
...
QUANTUM_CONFIG = {
    ...
    "min_score_otc": 65,
    "cooldown": 300,
    "weights": {"structure": 20, "liquidity": 20, "order_block": 15, "fvg": 15, "volume": 5, "momentum": 20},
    "learning": {"min_trades": 50, "update_interval": 86400, "max_weight": 30, "min_weight": 5},
    "backtest": {"min_history": 100, "test_size": 0.3},
    "volatility_filter": {"min_volatility": 0.0005, "max_volatility": 0.01, "ideal_low": 0.001, "ideal_high": 0.005, "score_bonus": 5, "score_penalty": 10, "reject_low": 0.00005, "reject_high": 0.012}
}

QUANTUM_SIGNAL_NAMES = {1: ("Quantum Bronze 🧠🥉", "QUANTUM BRONZE"), 2: ("Quantum Silver 🧠🥈", "QUANTUM SILVER"), 3: ("Quantum Gold 🧠🥇", "QUANTUM GOLD"), 4: ("Quantum Elite 🧠👑", "QUANTUM ELITE")}
...
if not IQ_EMAIL or not IQ_PASSWORD:
    raise ValueError("❌ IQ_EMAIL and IQ_PASSWORD required!")
...
FILES = {
    "settings_live.json": {},
    "settings_otc.json": {},
    "king_weights.json": {"structure": 20, "sweep": 20, "trend": 15, "momentum": 10, "volatility": 10, "adx": 10, "rsi": 5, "stochastic": 5, "candle": 5},
...
DEFAULT_KING_WEIGHTS = {"structure": 20, "sweep": 20, "trend": 15, "momentum": 10, "volatility": 10, "adx": 10, "rsi": 5, "stochastic": 5, "candle": 5}
...
def load_settings(market_type="live"):
    ...
    default = {
        "adx_threshold": 22, "rsi_low_call": 30, "rsi_high_call": 50,
        "rsi_low_put": 50, "rsi_high_put": 70, "sweep_threshold": 0.0003,
...
SIGNAL_NAMES = {2: ("قوية جداً 🔵", "VERY STRONG"), 3: ("قوية ماكس 🟣", "STRONG MAX"), 4: ("قوية سوبر ماكس 🟠", "STRONG SUPER MAX"), 5: ("ماكس 🔥", "MAX"), 6: ("سوبر ماكس 👑", "SUPER MAX")}
SIGNAL_EMOJIS = {2: "🔵", 3: "🟣", 4: "🟠", 5: "🔥", 6: "👑"}

KING_SIGNAL_NAMES = {1: ("King Bronze 🥉", "KING BRONZE"), 2: ("King Silver 🥈", "KING SILVER"), 3: ("King Gold 👑", "KING GOLD"), 4: ("King Elite 👑🔥", "KING ELITE")}
KING_EMOJIS = {1: "🥉", 2: "🥈", 3: "👑", 4: "👑🔥"}

SMC_SIGNAL_NAMES = {1: ("SMC Bronze 🥉", "SMC BRONZE"), 2: ("SMC Silver 🥈", "SMC SILVER"), 3: ("SMC Gold 🥇", "SMC GOLD"), 4: ("SMC Elite 🏆", "SMC ELITE")}
SMC_EMOJIS = {1: "🥉", 2: "🥈", 3: "🥇", 4: "🏆"}

PRO_SIGNAL_NAMES = {1: ("Pro Bronze 🥉", "PRO BRONZE"), 2: ("Pro Silver 🥈", "PRO SILVER"), 3: ("Pro Gold 🥇", "PRO GOLD"), 4: ("Pro Elite 🔥", "PRO ELITE")}
PRO_EMOJIS = {1: "🥉", 2: "🥈", 3: "🥇", 4: "🔥"}
...
# ✅ FIX: استخدام htf_data في الدالة
def get_regime_badge(strategy_name, regime, htf_data=None, indicator_counts=None):
    ...
    # ✅ استخدام htf_data اللي كان مش مستخدم
    if htf_data and isinstance(htf_data, dict):
        htf_trend = htf_data.get("trend")
        htf_conf = htf_data.get("confidence", 0)
        htf_struct = htf_data.get("structure", "unknown")
        if htf_trend:
            enhancers += f"\n🔭 [HTF-1H]: {htf_trend} | Structure={htf_struct} | Conf={htf_conf}%"

    badges = {
        'original': {'trending': "🌊 السوق *ترندي قوي* — الاستراتيجية الأصلية *ممتازة* هنا" + enhancers, 'ranging': "↔️ السوق *متراوح* — الاستراتيجية الأصلية *متوسطة* هنا" + enhancers, 'high_vol': "⚡ تقلب عالي — الاستراتيجية الأصلية *جيدة*" + enhancers, 'low_vol': "😴 تقلب منخفض — الاستراتيجية الأصلية *ضعيفة* هنا" + enhancers, 'mixed': "🌫️ سوق مختلط — الاستراتيجية الأصلية *عادية*" + enhancers, 'unknown': "❓ نوع السوق غير واضح" + enhancers},
        'king': {'trending': "🌊 السوق *ترندي قوي* — King Strategy *ممتازة* 👑" + enhancers, 'ranging': "↔️ السوق *متراوح* — King Strategy *متوسطة*" + enhancers, 'high_vol': "⚡ تقلب عالي — King Strategy *جيدة*" + enhancers, 'low_vol': "😴 تقلب منخفض — King Strategy *ضعيفة*" + enhancers, 'mixed': "🌫️ سوق مختلط — King Strategy *عادية*" + enhancers, 'unknown': "❓ نوع السوق غير واضح" + enhancers},
        'smart': {'trending': "🌊 السوق *ترندي قوي* — SMC Strategy *جيدة*" + enhancers, 'ranging': "↔️ السوق *متراوح* — SMC Strategy *ضعيفة* هنا" + enhancers, 'high_vol': "⚡ تقلب عالي — SMC Strategy *ممتازة* 🏆" + enhancers, 'low_vol': "😴 تقلب منخفض — SMC Strategy *ضعيفة*" + enhancers, 'mixed': "🌫️ سوق مختلط — SMC Strategy *عادية*" + enhancers, 'unknown': "❓ نوع السوق غير واضح" + enhancers},
        'pro': {'trending': "🌊 السوق *ترندي قوي* — Pro Strategy *متوسطة*" + enhancers, 'ranging': "↔️ السوق *متراوح* — Pro Strategy *ممتازة* 🔥" + enhancers, 'high_vol': "⚡ تقلب عالي — Pro Strategy *متوسطة*" + enhancers, 'low_vol': "😴 تقلب منخفض — Pro Strategy *ضعيفة*" + enhancers, 'mixed': "🌫️ سوق مختلط — Pro Strategy *جيدة*" + enhancers, 'unknown': "❓ نوع السوق غير واضح" + enhancers},
        'quantum': {'trending': "🌊 السوق *ترندي قوي* — Quantum Strategy *ممتازة* 🧠" + enhancers, 'ranging': "↔️ السوق *متراوح* — Quantum Strategy *❌ مرفوضة*" + enhancers, 'high_vol': "⚡ تقلب عالي — Quantum Strategy *جيدة*" + enhancers, 'low_vol': "😴 تقلب منخفض — Quantum Strategy *❌ مرفوضة*" + enhancers, 'mixed': "🌫️ سوق مختلط — Quantum Strategy *جيدة*" + enhancers, 'unknown': "❓ نوع السوق غير واضح — Quantum Strategy *⏸️ متوقفة*" + enhancers}
    }
...
# ✅ FIX #1: تطبيق فلاتر الـ Regime بشكل صحيح
def check_regime_for_strategy(strategy_name, regime):
    """
    يتحقق إذا كانت الاستراتيجية مسموحة في هذا الـ regime.
    - 'unknown' بيسمح للمرور (تفادي حجب كل الإشارات لما التحليل يفشل)
    - 'mixed' بيتعامل حسب كل استراتيجية
    """
...
def get_time_quality(strategy_name):
    ...
    time_ranges = {
        'original': {'best': [(15*60, 18*60)], 'good': [(10*60, 14*60)]},
        'king': {'best': [(10*60, 12*60)], 'good': [(15*60, 17*60)]},
        'smart': {'best': [(11*60, 15*60)], 'good': [(16*60, 19*60)]},
        'pro': {'best': [(15*60, 18*60)], 'good': [(10*60, 14*60)]},
        'quantum': {'best': [(10*60, 14*60), (15*60, 18*60)], 'good': [(9*60, 10*60), (18*60, 20*60)]}
    }
...
# ✅ FIX #3: استخدام get_iq_time() للتخزين بدل time.time()
def send_final_signal(pair, direction, signal_name, score, duration_text, indicators, strategy_name, regime="unknown", signal_level=None, htf_data=None, indicator_counts=None):
    ...
    with data_lock:
        if msg_hash in state.sent_final_signals:
            return None
        state.sent_final_signals[msg_hash] = get_iq_time()  # ✅ FIX
...
def evaluate_filters(trades, market_type=None):
    ...
    for fname, stat in filter_stats.items():
        ...
        if total >= 10:
            ...
            results[fname] = {"win": stat["win"], "loss": stat["loss"], "total": total, "wr": round(wr, 1), "worth": worth}
...
def rank_pairs(trades, market_type=None):
    ...
    for pair, data in pair_data.items():
        ...
        rankings.append({"pair": pair, "wr": round(wr, 1), "total": total, "profit_factor": round(profit_factor, 2), "stability": round(stability, 1), "score": round(score, 1)})
...
def analyze_hours(trades, market_type=None):
    ...
    for h, stat in hour_stats.items():
        if stat["total"] >= 5:
            results[h] = {"win": stat["win"], "loss": stat["loss"], "total": stat["total"], "wr": round((stat["win"] / stat["total"]) * 100, 1)}
    return dict(sorted(results.items(), key=lambda x: x[1]["wr"], reverse=True))

def analyze_confidence_calibration(trades):
    if not trades:
        return {}
    score_buckets = {"80-84": {"trades": [], "expected_wr": 82}, "85-89": {"trades": [], "expected_wr": 87}, "90-94": {"trades": [], "expected_wr": 92}, "95-100": {"trades": [], "expected_wr": 97}}
    for t in trades:
        score = t.get("score", 0)
        if 80 <= score <= 84:
    ...
    for bucket, data in score_buckets.items():
        ...
        if len(trades_in_bucket) >= 10:
            ...
            calibration[bucket] = {"total": len(trades_in_bucket), "actual_wr": round(actual_wr, 1), "expected_wr": expected_wr, "diff": round(diff, 1), "status": "✅ متوازن" if abs(diff) <= 5 else ("⚠️ مبالغ" if diff < -5 else "🔥 أقوى من المتوقع")}
    return calibration
...
def grid_search_optimization(trades, strategy="king", market_type=None):
    ...
    if best_adx != 22 and best_adx_count >= 30:
        proposals.append({"filter": "ADX", "current": 22, "proposed": best_adx, "reason": f"WR يتحسن لـ {best_adx_wr:.1f}% مع ADX ≥ {best_adx} (عينة: {best_adx_count})", "impact": "يقلل الإشارات قليلاً ويرفع الجودة"})
    best_rsi_low, best_rsi_high = 45, 60
    best_rsi_wr = 0
    best_rsi_count = 0
    for low in range(40, 50, 2):
        for high in range(55, 65, 2):
            subset = [t for t in king_trades if t.get("direction") == "CALL" and low <= t.get("indicators", {}).get("rsi", 0) <= high]
            if len(subset) >= 15:
                wins = sum(1 for t in subset if t.get("outcome") == "win")
                wr = (wins / len(subset)) * 100
    ...
    if (best_rsi_low, best_rsi_high) != (45, 60) and best_rsi_count >= 20:
        proposals.append({"filter": "RSI CALL", "current": "45–60", "proposed": f"{best_rsi_low}–{best_rsi_high}", "reason": f"WR يتحسن لـ {best_rsi_wr:.1f}% (عينة: {best_rsi_count})", "impact": "تعديل دقيق لنطاق RSI"})
...
def optimize_weights(trades):
    ...
    for fname in current_weights.keys():
        ...
        if len(with_filter) >= 20 and len(without_filter) >= 20:
            ...
            filter_performance[fname] = {"wr_with": wr_with, "wr_without": wr_without, "diff": wr_with - wr_without, "count": len(with_filter)}
    ...
    return {"old_weights": current_weights, "new_weights": new_weights, "adjustments": adjustments, "filter_performance": filter_performance}, "تم تحديث الأوزان"
...
def calculate_feature_importance(trades, strategy="king"):
    ...
    for fname in filter_names:
        ...
        if len(with_filter) >= 20 and len(without_filter) >= 20:
            ...
            importance[fname] = {"importance": round(imp, 2), "wr_with": round(wr_with, 1), "wr_without": round(wr_without, 1), "count": len(with_filter)}
    if importance:
...
def optimize_weights_feature_importance(trades):
    ...
    return {"old_weights": old_weights, "new_weights": new_weights, "importance": result["importance"], "baseline_wr": result["baseline_wr"], "adjustments": adjustments}, "تم تحديث الأوزان بناءً على Feature Importance"
...
def generate_report(trades, period="daily", market_type=None):
    ...
    expectancy = (PAYOUT_RATIO * (wr/100)) - (1 * (1 - wr/100))
    ...
    return {
        "period": period, "market_type": market_type or "all", "total_trades": total, "wins": wins, "losses": losses,
        "win_rate": round(wr, 1), "profit_factor": round(pf, 2), "max_win_streak": max_win_streak, "max_loss_streak": max_loss_streak,
        "expectancy": round(expectancy, 3), "best_pair": best_pair, "worst_pair": worst_pair, "best_hour": best_hour, "worst_hour": worst_hour,
        ...
        "filter_eval": evaluate_filters(trades), "calibration": analyze_confidence_calibration(trades),
...
def generate_daily_sheet(trades):
    ...
    strategy_names = {"original": "الأصلية", "king": "👑 King", "smart": "🏆 SMC", "pro": "🔥 Pro", "quantum": "🧠 Quantum"}
    stats = {}
    for strategy in strategies:
        ...
        total = len(st_trades)
        stats[strategy] = {"name": strategy_names.get(strategy, strategy), "total": total, "wins": wins, "losses": losses, "ties": ties, "wr": round((wins / total * 100), 1) if total > 0 else 0}
    if not stats:
    ...
    return {"strategies": stats, "total": total_all, "wins": total_wins, "losses": total_losses, "ties": total_ties, "wr": total_wr, "date": datetime.now(CAIRO_TZ).strftime('%d/%m/%Y')}, None
...
def generate_and_send_optimization_proposal():
    ...
    if proposals:
        ...
        for p in proposals:
            msg += f"\n🔹 *{p['filter']}*\n   الحالي: `{p['current']}`\n   المقترح: `{p['proposed']}`\n   السبب: {p['reason']}\n   التأثير: {p['impact']}\n"
    if weight_result:
        msg += f"\n⚖️ *تعديلات أوزان King Strategy:*\n📊 Baseline WR: {weight_result.get('baseline_wr', 'N/A')}%\n"
    ...
    msg += f"\n━━━━━━━━━━━━━━━━━━━━\n✅ *للموافقة:* رد `موافق`\n❌ *للرفض:* رد `رفض`\n⏳ *متاح 24 ساعة*"
    proposal_data = {"timestamp": get_iq_time(), "proposals": proposals or [], "weight_result": weight_result, "status": "pending", "message": msg}
    with data_lock:
...
def run_walk_forward_validation(trades, strategy="king", market_type="live"):
    ...
    best_config = None
    best_train_wr = 0
    for adx_t in [20, 22, 24, 26, 28]:
        for rsi_l in [40, 42, 45, 48]:
            for rsi_h in [55, 58, 60, 62]:
                filtered = []
                for t in train_set:
    ...
    result = {"market_type": market_type, "strategy": strategy, "train_size": len(train_set), "test_size": len(test_set), "baseline_wr": round(baseline_wr, 1), "new_wr": round(test_wr, 1), "improvement": round(improvement, 1), "best_config": best_config, "approved": improvement > 2}
...
def run_monte_carlo(trades, strategy="king", market_type=None):
    ...
    return {"strategy": strategy, "market_type": market_type, "trades": n, "simulations": MONTE_CARLO_SIMULATIONS, "baseline_wr": round(baseline_wr, 1), "mc_mean_wr": round(wr_mean, 1), "mc_wr_std": round(wr_std, 1), "mc_wr_5th": round(wr_5th, 1), "mc_wr_95th": round(wr_95th, 1), "mc_mean_dd": round(dd_mean, 1), "mc_dd_95th": round(dd_95th, 1), "risk_of_ruin": round(risk_of_ruin, 1), "stability": round(stability, 1), "status": "✅ مستقر" if stability >= 80 and risk_of_ruin < 5 else ("⚠️ متوسط" if stability >= 60 else "🔴 ضعيف")}, "تم"
...
def format_monte_carlo_message(result):
    ...
    return (
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 *نسبة الربح:*\n   Baseline: {result['baseline_wr']}%\n   MC Mean: {result['mc_mean_wr']}% (±{result['mc_wr_std']}%)\n   5th–95th: {result['mc_wr_5th']}% – {result['mc_wr_95th']}%\n\n"
        f"📉 *Max Drawdown:*\n   Mean: {result['mc_mean_dd']} صفقات\n   95th: {result['mc_dd_95th']} صفقات\n\n"
...
def format_monte_carlo_summary(results_dict):
    ...
    msg = "🎲 *Monte Carlo — ملخص شهري*\n" + f"📅 {datetime.now(CAIRO_TZ).strftime('%d/%m/%Y %I:%M %p')}\n━━━━━━━━━━━━━━━━━━━━\n\n"
    for market in ["live", "otc"]:
        ...
        for strategy, result in market_data.items():
            msg += f"  📌 `{strategy}`:\n     ⚠️ Risk of Ruin: {result.get('risk_of_ruin', 'N/A')}%\n     📉 Max Drawdown: {result.get('mc_dd_95th', 'N/A')} صفقات\n     🛡️ الاستقرار: {result.get('stability', 'N/A')}%\n     📊 Baseline WR: {result.get('baseline_wr', 'N/A')}%\n     {result.get('status', '')}\n"
        msg += "\n"
    msg += "━━━━━━━━━━━━━━━━━━━━"
    return msg
...
def get_htf_market_regime(pair):
    ...
    try:
        ...
        if ema_strength >= 1.0:
            confidence += 10
        result = {"regime": regime, "trend": trend_dir, "structure": structure, "confidence": min(confidence, 100), "adx": float(adx), "atr": float(atr), "bbw": float(bbw), "supertrend": supertrend_signal, "macd": macd_signal, "ema_alignment": ema_trend, "trend_strength": trend_strength, "votes": {"call": call_votes, "put": put_votes, "total": total_votes}}
        with data_lock:
...
def analyze_volatility_filter(volatility):
    ...
    if volatility < reject_low:
        return {'status': 'REJECT', 'reason': f'⚠️ تقلب منخفض جداً ({volatility:.4f}) - السوق راكد', 'score_adjust': 0, 'can_enter': False}
    if volatility > reject_high:
        return {'status': 'REJECT', 'reason': f'⚠️ تقلب عالي جداً ({volatility:.4f}) - السوق عنيف', 'score_adjust': 0, 'can_enter': False}
    if volatility < ideal_low:
        return {'status': 'WARNING', 'reason': f'⚠️ تقلب منخفض ({volatility:.4f}) - خصم {score_penalty} نقطة', 'score_adjust': -score_penalty, 'can_enter': True, 'emoji': '😴'}
    if ideal_low <= volatility <= ideal_high:
        return {'status': 'IDEAL', 'reason': f'✅ تقلب مثالي ({volatility:.4f}) - مكافأة +{score_bonus} نقطة', 'score_adjust': score_bonus, 'can_enter': True, 'emoji': '🎯'}
    if volatility < reject_high:
        penalty = score_penalty // 2
        return {'status': 'WARNING', 'reason': f'⚠️ تقلب عالي ({volatility:.4f}) - خصم {penalty} نقطة', 'score_adjust': -penalty, 'can_enter': True, 'emoji': '⚡'}
    return {'status': 'OK', 'reason': f'تقلب متوسط ({volatility:.4f})', 'score_adjust': 0, 'can_enter': True, 'emoji': '📊'}
...
def detect_order_block_quantum(df, curr):
    try:
        ...
        for i in range(start, len(df)):
            candle = df.iloc[i]
            prev = df.iloc[i-1]
            if (candle['Close'] > candle['Open'] and candle['Close'] > prev['High'] and prev['Close'] < prev['Open'] and (candle['Close'] - candle['Open']) > abs(prev['Close'] - prev['Open']) * 1.5):
                if prev['Low'] <= price <= prev['High'] * 1.002:
                    return "BULLISH"
        for i in range(start, len(df)):
            candle = df.iloc[i]
            prev = df.iloc[i-1]
            if (candle['Close'] < candle['Open'] and candle['Close'] < prev['Low'] and prev['Close'] > prev['Open'] and (candle['Open'] - candle['Close']) > abs(prev['Close'] - prev['Open']) * 1.5):
...
def calculate_confidence_score_quantum(structure, liquidity, order_block, fvg, volume, momentum):
    ...
    put = 0
    reasons = []
    weights = QUANTUM_CONFIG.get("weights", {"structure": 20, "liquidity": 20, "order_block": 15, "fvg": 15, "volume": 5, "momentum": 20})
...
def update_quantum_weights(trade_history):
    ...
    if updates:
        quantum_weights_history.append({"timestamp": get_iq_time(), "updates": updates, "weights": QUANTUM_CONFIG["weights"].copy()})
        try:
...
def feature_importance_quantum(trade_history):
    ...
    feature_names = {'structure': 'Structure', 'liquidity': 'Liquidity Sweep', 'order_block': 'Order Block', 'fvg': 'FVG', 'volume': 'Volume', 'momentum': 'Momentum'}
    ...
    for feature in features:
        ...
        if len(feature_trades) >= 20:
            ...
            importance[feature_names[feature]] = {"winrate": round(winrate, 1), "coverage": round(coverage, 1), "importance": round(importance_score, 1), "trades": len(feature_trades)}
        else:
            importance[feature_names[feature]] = {"winrate": 0, "coverage": 0, "importance": 0, "trades": 0, "status": "بيانات غير كافية"}
    return dict(sorted(importance.items(), key=lambda x: x[1]['importance'] if isinstance(x[1], dict) else 0, reverse=True))
...
def handle_quantum_command(command):
    ...
    if cmd == "/quantum_weights":
        ...
        QUANTUM_CONFIG["weights"] = {"structure": 20, "liquidity": 20, "order_block": 15, "fvg": 15, "volume": 5, "momentum": 20}
...
def analyze_pair_quantum(pair, timeframe="5m"):
    ...
    # ✅ FIX #1: تطبيق فلتر Regime
    if not check_regime_for_strategy('quantum', regime):
        logger.info(f"🛑 Quantum {pair}: regime={regime} غير مسموح")
        return None
    ...
    # ✅ FIX #4: نقل API calls خارج data_lock
    if 270 <= csec <= 280:
        should_send = False
        with data_lock:
            if pair_key not in state.quantum_alerted_pairs:
                state.pending_alerts[pending_key] = {'direction': result['direction'], 'level': level, 'signal_name': signal_name_ar, 'score': final_score, 'alert_time': iq_now, 'strategy': 'quantum'}
        ...
        if should_send:
            ...
            msg = (
                f"⚠️ *تنبيه مبكر — {signal_name_ar}*\nالزوج: `{pair}` [5 دقائق]\nالاتجاه: *{da}*\n📊 النقاط: *{final_score}/100* (معدلة)\n⏱️ *صفقة قادمة خلال 20 ثانية...*\n🔄 *جاري التحقق من الشروط النهائية...*\n━━━━━━━━━━━━\n🕐 *الوقت:* {time_quality}\n📍 {regime_badge}\n⚛️ Kalman: {smoothed_price:.5f} | {vol_emoji} {vol_status}"
            )
            send_telegram_message(msg)
        return None
    if not (280 <= csec <= 299):
    ...
    if csec >= 293:
        ...
        new_trade = _build_trade_dict(
            pair=pair, direction=result['direction'], entry_price=price, expire_offset=300,
            is_king=False, is_martingale=False, signal_level=level, signal_name=signal_name_ar, score=final_score,
            filters={'structure': structure is not None, 'liquidity': liquidity is not None, 'order_block': order_block is not None, 'fvg': fvg is not None, 'volume': volume, 'momentum': momentum is not None, 'volatility_ok': vol_filter['can_enter']},
            indicators={'score': final_score, 'original_score': result['score'], 'score_adjust': vol_filter['score_adjust'], 'reasons': result['reasons'], 'market': regime, 'structure': str(structure) if structure else 'None', 'liquidity': str(liquidity) if liquidity else 'None', 'order_block': str(order_block) if order_block else 'None', 'fvg': str(fvg) if fvg else 'None', 'kalman_price': round(smoothed_price, 5), 'volatility': round(volatility, 4), 'volatility_status': vol_filter['status']},
            strategy='quantum'
        )
        if not add_trade_atomic(new_trade):
            return None
        htf_data = get_htf_market_regime(pair)
        final_signal = send_final_signal(pair, result['direction'], signal_name_ar, final_score, duration_text, indicators_str, 'quantum', regime=regime, signal_level=level, htf_data=htf_data, indicator_counts=indicator_counts)
...
ORIGINAL_WEIGHTS = {"trend": 20, "momentum": 20, "structure": 15, "volume": 15, "candle": 15, "zone": 10, "adx": 5}

def evaluate_signal_strength_enhanced(direction, curr, prev, df, price, alma9, alma50, stoch_k, stoch_d, rsi, volume, vol_ma, atr, adx, bbw, roc, near_sup, near_res, structure=None, htf_regime=None):
    score = 0
    reasons = []
    w = ORIGINAL_WEIGHTS
    ...
    if direction == "CALL":
        stoch_ok = stoch_k > stoch_d and stoch_k > 50
        roc_ok = roc > 0.03
    else:
        stoch_ok = stoch_k < stoch_d and stoch_k < 50
        roc_ok = roc < -0.03
...
def analyze_pair(pair, timeframe="5m"):
    ...
    # ✅ FIX #1: تطبيق فلتر Regime
    if not check_regime_for_strategy('original', regime):
        logger.info(f"🛑 {pair}: Original — regime={regime} غير مسموح")
        return None
    ...
    atr_avg = atr_series.tail(20).mean()
    if atr < atr_avg * 0.4:
        logger.info(f"🛑 {pair}: تقلب منخفض")
        return None
    if atr > atr_avg * 3.5:
        logger.info(f"⚠️ {pair}: تقلب عالي — مستمر بحذر")
    # ✅ FIX #5: تعليق صحيح (كان 1.2/1.5/1.8 متضاربين)
    if curr['Volume'] <= vol_ma * 1.2:
        logger.info(f"🛑 {pair}: حجم ضعيف (Vol < MA*1.2)")
        return None
    ...
    strength, reasons = evaluate_signal_strength_enhanced(potential_direction, curr, prev, df, price, alma9, alma50, stoch_k, stoch_d, rsi, volume, vol_ma, atr, adx, bbw, roc, near_sup, near_res, structure=structure, htf_regime=htf)
    if strength < 5:
    ...
    if 270 <= csec <= 280:
        ...
        with data_lock:
            if pair not in state.alerted_pairs:
                state.pending_alerts[pair] = {'direction': potential_direction, 'strength': strength, 'signal_name': signal_name_ar, 'score': strength * 16, 'alert_time': iq_now, 'strategy': 'original'}
    ...
    if 280 <= csec <= 299:
        ...
        if csec >= 293:
            ...
            indicator_counts = get_indicator_counts(pair, df)
            htf_data = get_htf_market_regime(pair)
            final_signal = send_final_signal(pair, potential_direction, signal_name_ar, strength * 16, duration_text, indicators_str, 'original', regime=regime, signal_level=strength, htf_data=htf_data, indicator_counts=indicator_counts)
            if final_signal is None:
            ...
            new_trade = _build_trade_dict(
                pair=pair, direction=potential_direction, entry_price=curr['Close'], expire_offset=300,
                is_king=False, is_martingale=False, signal_level=strength, signal_name=signal_name_ar, score=strength * 16,
                filters={'alma_cross': (a9p <= a50p and a9c > a50c) if potential_direction == "CALL" else (a9p >= a50p and a9c < a50c), 'price_above_alma': price > alma9 if potential_direction == "CALL" else price < alma9, 'stoch_aligned': stoch_k > stoch_d if potential_direction == "CALL" else stoch_k < stoch_d, 'rsi_zone': (28 <= rsi <= 65) if potential_direction == "CALL" else (35 <= rsi <= 72), 'near_sr': near_sup if potential_direction == "CALL" else near_res, 'volume_ok': volume >= vol_ma * 1.5, 'adx_ok': adx >= 20, 'bbw_ok': bbw >= 0.001, 'atr_ok': atr >= (price * 0.00025), 'structure_ok': structure in ["BULLISH", "BEARISH"]},
                indicators={'adx': float(adx), 'rsi': float(rsi), 'bbw': float(bbw), 'atr': float(atr), 'roc': float(roc), 'stoch_k': float(stoch_k), 'stoch_d': float(stoch_d), 'structure': str(structure) if structure else 'None', 'reasons': reasons},
                strategy='original'
            )
...
def analyze_pair_king(pair, timeframe="5m"):
    ...
    # ✅ FIX #1: تطبيق فلتر Regime
    if not check_regime_for_strategy('king', regime):
        logger.info(f"🛑 King {pair}: regime={regime} غير مسموح")
        return None

    df = detect_swings(df, window=2)
    structure, last_sh_idx, last_sl_idx = get_market_structure(df, lookback=30)

    # ✅ FIX #2: King لا يدخل PUT عند NEUTRAL — يستخدم HTF trend أو يمتنع
    if structure == "BULLISH":
        potential_direction = "CALL"
    elif structure == "BEARISH":
        potential_direction = "PUT"
    else:
        # NEUTRAL - نجرب HTF trend، لو مش موجود نمتنع
        htf_check = get_htf_market_regime(pair)
        htf_trend_check = htf_check.get("trend") if htf_check else None
        if htf_trend_check in ("CALL", "PUT"):
            potential_direction = htf_trend_check
            logger.info(f"ℹ️ King {pair}: NEUTRAL — استخدام HTF trend={htf_trend_check}")
        else:
            logger.info(f"🛑 King {pair}: NEUTRAL ولا يوجد HTF trend — مرفوض")
            return None
    ...
    score = calculate_king_score(
        structure_ok=(structure in ["BULLISH", "BEARISH"]), sweep_ok=sweep_ok, trend_ok=trend_ok,
        momentum_ok=momentum_ok, volatility_ok=volatility_ok, adx_ok=adx_ok, rsi_ok=rsi_ok,
        stoch_ok=stoch_ok, candle_ok=candle_ok
    )
    level = get_adaptive_king_level(score, market_type=market_type)
    if level == 0:
        logger.info(f"🛑 King {pair}: Score={score} < threshold")
        return None
    htf_trend = get_king_htf_trend(pair)
    if htf_trend is not None and htf_trend != potential_direction:
        logger.warning(f"⚠️ King HTF عكسي لـ {pair} لكن الإشارة مستمرة")
    is_trending = (potential_direction == "CALL" and htf_trend == "CALL") or (potential_direction == "PUT" and htf_trend == "PUT")
    ...
    if 270 <= csec <= 280:
        ...
        with data_lock:
            if pair_key not in state.king_alerted_pairs:
                state.pending_alerts[f"king_{pair}"] = {'direction': potential_direction, 'level': level, 'signal_name': signal_name_ar, 'score': score, 'alert_time': iq_now, 'strategy': 'king'}
    ...
    if 280 <= csec <= 299:
        ...
        if csec >= 293:
            ...
            new_trade = _build_trade_dict(
                pair=pair, direction=potential_direction, entry_price=curr['Close'], expire_offset=300,
                is_king=True, is_martingale=False, signal_level=level, signal_name=signal_name_ar, score=score,
                filters={'structure_ok': structure in ["BULLISH", "BEARISH"], 'sweep_ok': sweep_ok, 'trend_ok': trend_ok, 'momentum_ok': momentum_ok, 'volatility_ok': volatility_ok, 'adx_ok': adx_ok, 'rsi_ok': rsi_ok, 'stoch_ok': stoch_ok, 'candle_ok': candle_ok},
                indicators={'adx': float(adx), 'rsi': float(rsi), 'roc': float(roc), 'atr': float(atr), 'bbw': float(bbw), 'stoch_k': float(stoch_k), 'stoch_d': float(stoch_d), 'sweep_threshold_used': float(sweep_threshold)},
                strategy='king'
            )
            if not add_trade_atomic(new_trade):
            ...
            htf_data = get_htf_market_regime(pair)
            final_signal = send_final_signal(pair, potential_direction, signal_name_ar, score, duration_text, indicators_str, 'king', regime=regime, htf_data=htf_data, indicator_counts=indicator_counts)
            if final_signal is None:
                logger.info(f"⛔ King {pair}: تم إرسالها مسبقاً")
                return None
...
def detect_order_blocks(df, lookback=30):
    ...
    for i in range(3, len(recent)):
        c, p = recent.iloc[i], recent.iloc[i-1]
        if c['Close'] > c['Open'] and c['Close'] > p['High'] and p['Close'] < p['Open'] and (c['Close']-c['Open']) > abs(p['Close']-p['Open'])*1.5:
            obs['bull'].append({'high': p['High'], 'low': p['Low'], 'idx': i-1})
        elif c['Close'] < c['Open'] and c['Close'] < p['Low'] and p['Close'] > p['Open'] and (c['Open']-c['Close']) > abs(p['Close']-p['Open'])*1.5:
            obs['bear'].append({'high': p['High'], 'low': p['Low'], 'idx': i-1})
    return obs

def detect_breaker_blocks(df, lookback=40):
    df = detect_swings(df, window=2)
    swings = df[df['is_swing_high'] | df['is_swing_low']].tail(lookback)
    breakers = {'bull': [], 'bear': []}
    if len(swings) < 4: return breakers
    for i in range(3, len(swings)):
        s = [swings.iloc[j] for j in range(i-3, i+1)]
        if s[0]['is_swing_low'] and s[1]['is_swing_high'] and s[2]['is_swing_low'] and s[3]['is_swing_high']:
            if s[2]['Low'] < s[0]['Low'] and s[3]['High'] > s[1]['High']:
                breakers['bear'].append({'high': s[2]['High'], 'low': s[2]['Low'], 'idx': swings.index[i-2]})
        elif s[0]['is_swing_high'] and s[1]['is_swing_low'] and s[2]['is_swing_high'] and s[3]['is_swing_low']:
            if s[2]['High'] > s[0]['High'] and s[3]['Low'] < s[1]['Low']:
                breakers['bull'].append({'high': s[2]['High'], 'low': s[2]['Low'], 'idx': swings.index[i-2]})
    return breakers
...
def analyze_pair_smc(pair, timeframe="5m"):
    ...
    # ✅ FIX #1: تطبيق فلتر Regime
    if not check_regime_for_strategy('smart', regime):
        logger.info(f"🛑 SMC {pair}: regime={regime} غير مسموح")
        return None
    ...
    if 270 <= csec <= 280:
        ...
        with data_lock:
            ...
            if pair_key not in state.smart_alerted_pairs:
                state.pending_alerts[f"smart_{pair}"] = {'direction': bias, 'level': level, 'signal_name': name, 'score': score, 'alert_time': iq_now, 'strategy': 'smart'}
    ...
    if csec >= 293:
        ...
        new_trade = _build_trade_dict(
            pair=pair, direction=bias, entry_price=price, expire_offset=300,
            is_king=False, is_martingale=False, signal_level=level, signal_name=name, score=score,
            filters={'structure': structure in ["BULLISH", "BEARISH"], 'sweep': sweep_ok, 'ob': ob_hit, 'breaker': bb_hit, 'fvg': fvg_hit},
            indicators={'rsi': float(rsi), 'score': score, 'conf': conf, 'htf': htf, 'mtf': mtf},
            strategy='smart'
        )
        if not add_trade_atomic(new_trade):
        ...
        htf_data = get_htf_market_regime(pair)
        final_signal = send_final_signal(pair, bias, name, score, duration_text, indicators_str, 'smart', regime=regime, htf_data=htf_data, indicator_counts=indicator_counts)
        if final_signal is None:
            logger.info(f"⛔ SMC {pair}: تم إرسالها مسبقاً")
            return None
...
def analyze_pair_pro(pair, timeframe="5m"):
    ...
    # ✅ FIX #1: تطبيق فلتر Regime
    if not check_regime_for_strategy('pro', regime):
        logger.info(f"🛑 Pro {pair}: regime={regime} غير مسموح")
        return None
    ...
    if direction is None or score < 65:
        logger.info(f"🛑 Pro {pair}: Score={score} < 65 أو لا يوجد اتجاه")
        return None
    if score >= 95: level = 4
    ...
    if 270 <= csec <= 280:
        ...
        with data_lock:
            if pair not in state.pa_alerted_pairs:
                state.pending_alerts[f"pro_{pair}"] = {'direction': direction, 'level': level, 'signal_name': name_ar, 'score': score, 'alert_time': iq_now, 'strategy': 'pro'}
    ...
    if csec >= 293:
        ...
        new_trade = _build_trade_dict(
            pair=pair, direction=direction, entry_price=curr['Close'], expire_offset=300,
            is_king=False, is_martingale=False, signal_level=level, signal_name=name_ar, score=score,
            filters={'structure': structure, 'rejection': True, 'volume_ok': vol_ok, 'sweep': (curr['Low'] < last_sup and curr['Close'] > last_sup) if direction == "CALL" else (curr['High'] > last_res and curr['Close'] < last_res)},
            indicators={'score': score, 'upper_wick': float(upper_wick), 'lower_wick': float(lower_wick), 'body': float(body), 'volume': float(curr['Volume']), 'vol_ma': float(vol_ma)},
            strategy='pro'
        )
        if not add_trade_atomic(new_trade):
        ...
        htf_data = get_htf_market_regime(pair)
        final_signal = send_final_signal(pair, direction, name_ar, score, duration_text, indicators_str, 'pro', regime=regime, htf_data=htf_data, indicator_counts=indicator_counts)
        if final_signal is None:
            logger.info(f"⛔ Pro {pair}: تم إرسالها مسبقاً")
            return None
...
def _build_trade_dict(pair, direction, entry_price, expire_offset, is_king, is_martingale, signal_level, signal_name, score, filters, indicators, strategy):
    ...
    return {
        'pair': pair, 'timeframe': '5m', 'direction': direction, 'entry_price': entry_price,
        'expire_time': get_iq_time() + expire_offset, 'warned_loss': False,
        'is_martingale': is_martingale, 'is_king': is_king, 'signal_level': signal_level,
        'signal_name': signal_name, 'score': score, 'filters': filters_clean,
        'indicators': indicators_clean, 'hour': datetime.now(CAIRO_TZ).hour, 'strategy': strategy
    }
            ...
            for pair, stat in pair_stats.items():
                if stat["total"] >= 30:
                    ...
                    if wr < DISABLE_THRESHOLD and pair not in state.disabled_pairs:
                        ...
                        logger.warning(f"🚫 {pair} متوقف — WR: {wr:.1f}%")
        if newly_disabled:
    ...
    try:
        ...
        for strategy in ["original", "king", "smart", "pro", "quantum"]:
            ...
            if len(trades) >= 20:
                ...
                with data_lock:
                    state.strategy_scores[strategy] = {"win": wins, "loss": len(trades) - wins, "total": len(trades), "wr": round(wr, 1), "stability": round(stability, 1), "score": round(score, 1)}
...
def calculate_adaptive_threshold(trades, market_type="live"):
    ...
    if wr >= 80: threshold = 80
    elif wr >= 70: threshold = 85
    elif wr >= 60: threshold = 90
    elif wr >= 50: threshold = 95
    else: threshold = 100
...
# ✅ FIX #11: get_adaptive_king_level يستخدم adaptive_thresholds فعلياً
def get_adaptive_king_level(score, market_type="live"):
    with data_lock:
        threshold = state.adaptive_thresholds.get(market_type, ADAPTIVE_THRESHOLD_MIN)
    # الفلتر الأساسي: لو النقاط أقل من العتبة → مرفوض
    if score < threshold:
        return 0
    # بعد تجاوز العتبة، تحديد المستوى
...
def calculate_indicator_votes_for_df(df):
    ...
    try:
        ...
        if st_dir.iloc[-1] == 1: votes["CALL"] += 1
        elif st_dir.iloc[-1] == -1: votes["PUT"] += 1
        else: votes["NEUTRAL"] += 1
    except Exception:
        votes["NEUTRAL"] += 1
    try:
        macd_line, signal_line, histogram = calculate_macd(df['Close'])
        if macd_line.iloc[-1] > signal_line.iloc[-1]: votes["CALL"] += 1
        else: votes["PUT"] += 1
    except Exception:
        votes["NEUTRAL"] += 1
    try:
        ema_trend, _ = get_ema_alignment(df)
        if ema_trend == "CALL": votes["CALL"] += 1
        elif ema_trend == "PUT": votes["PUT"] += 1
        else: votes["NEUTRAL"] += 1
    except Exception:
        votes["NEUTRAL"] += 1
    try:
        ...
        if a9c > a50c and a9p > a50p: votes["CALL"] += 1
        elif a9c < a50c and a9p < a50p: votes["PUT"] += 1
        else: votes["NEUTRAL"] += 1
    except Exception:
        votes["NEUTRAL"] += 1
    return votes
...
def calculate_king_score(structure_ok, sweep_ok, trend_ok, momentum_ok, volatility_ok, adx_ok, rsi_ok, stoch_ok, candle_ok):
    with data_lock:
        w = dict(KING_WEIGHTS)
    score = 0
    if structure_ok: score += w.get('structure', 20)
    if sweep_ok: score += w.get('sweep', 20)
    elif adx_ok: score += 10
...
# ✅ FIX #7: توحيد الحد الأدنى للشموع مع الاستخدام (100 بدل 80)
def get_cached_df_smart(pair, tf, count):
    ...
    if data is not None:
        return data
    raw = get_cached_candles(pair, tf, count, max_age=15)
    if not raw or len(raw) < 100:  # ✅ FIX: كان 80 — وحّدناه مع الاستخدام الفعلي
        return None
    df = pd.DataFrame(raw)
    df.rename(columns={'open':'Open','max':'High','min':'Low','close':'Close','volume':'Volume'}, inplace=True)
...
def get_king_htf_trend(pair):
    ...
    now = get_iq_time()  # ✅ FIX: توحيد الوقت
    with data_lock:
...
# ✅ FIX #6: استخدام tolerance بدل ==
def check_tie(ep, fp):
    return abs(float(ep) - float(fp)) < 1e-8
        ...
        try:
            ...
            if time_left <= 0:
                ...
                if not candles or len(candles) < 2:
                    logger.warning("⏳ " + pair + ": شموع غير كافية للتقييم")
                    continue
                target_candle = None
                ...
                if target_candle is None:
                    ...
                    logger.warning("⚠️ " + pair + ": استخدام fallback للشمعة")
                fp = target_candle['close']
                candle_to = target_candle.get('to', 0)
                candle_from = target_candle.get('from', 0)
                logger.info("📊 RESULT DEBUG | " + str(pair) + " | Dir:" + str(direction) + " | EP:" + "{:.5f}".format(ep) + " | FP:" + "{:.5f}".format(fp))
                ...
                logger.info("📊 RESULT | " + str(pair) + " | Win:" + str(is_win) + " | Tie:" + str(is_tie) + " | Diff:" + "{:.4f}".format(diff_pct) + "% | Strategy:" + str(strategy))
                ...
                try:
                    log_trade({"timestamp": get_iq_time(), "pair": pair, "direction": direction, "strategy": strategy, "level": trade.get('signal_level', 0), "score": trade.get('score', 0), "entry_price": float(ep), "exit_price": float(fp), "outcome": "win" if is_win else ("tie" if is_tie else "loss"), "filters": trade.get('filters', {}), "indicators": trade.get('indicators', {}), "hour": trade.get('hour', datetime.now(CAIRO_TZ).hour), "day_of_week": datetime.now(CAIRO_TZ).weekday(), "is_martingale": is_mg, "is_king": is_king, "candle_to": candle_to, "candle_from": candle_from, "expire_time": trade['expire_time']})
                except Exception as e:
                    logger.error("خطأ في تسجيل الصفقة: " + str(e))
                if is_tie:
                    send_telegram_message("➖ *تعادل*\nالزوج: `" + pair + "` [5m]\n⏰ `" + ts + "`\nالدخول: " + "{:.5f}".format(ep) + " | الخروج: " + "{:.5f}".format(fp))
                elif is_mg:
        ...
        try:
            ...
            for market in ["live", "otc"]:
                ...
                if len(market_trades) >= WALK_FORWARD_MIN_TRADES and now - last_walk_forward > 1209600:
                    for strategy in ["original", "king", "smart", "pro", "quantum"]:
                        approved, wf_result, wf_msg = run_walk_forward_validation(market_trades, strategy=strategy, market_type=market)
                        send_telegram_message(f"🔬 *Walk Forward [{market.upper()}/{strategy.upper()}]*\n{wf_msg}")
                        if approved and wf_result:
                            config = wf_result.get("best_config", {})
                            send_telegram_message(f"📋 *إعدادات مقترحة:*\n   السوق: *{market.upper()}*\n   الاستراتيجية: *{strategy.upper()}*\n   ADX ≥ {config.get('adx', 'N/A')}\n   RSI CALL: {config.get('rsi_low', 'N/A')}–{config.get('rsi_high', 'N/A')}\n   RSI PUT: {100 - config.get('rsi_high', 'N/A')}–{100 - config.get('rsi_low', 'N/A')}\n\n✅ تم حفظها في `settings_{market}.json`")
...
# ✅ FIX #3: توحيد الوقت على get_iq_time()
def cleanup_memory():
    now = get_iq_time()  # ✅ FIX: كان time.time()
...
def get_pairs_for_today():
    day_of_week = datetime.now(CAIRO_TZ).weekday()
    if day_of_week in [5, 6]:
        return ["EURUSD-OTC", "GBPUSD-OTC", "USDJPY-OTC", "USDCHF-OTC", "EURJPY-OTC", "EURGBP-OTC", "AUDCAD-OTC", "GBPJPY-OTC"], "OTC (عطلة)"
    else:
        return ["EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCAD", "USDCHF", "EURJPY", "EURGBP", "AUDCAD", "AUDJPY", "CADJPY", "EURAUD", "GBPJPY", "EURCAD"], "عادي (سوق مفتوح)"
...
def run_bot():
    global API
    # ✅ FIX #10: init_log_files بتتنادى مرة واحدة بس في __main__
    API = connect_iqoption()
    init_quantum_system()
    ...
    send_telegram_message(
        ...
        f"✨ *v7.7:* إصلاح فلتر Regime + King NEUTRAL + توحيد الوقت + تحسين الأداء"
    )
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if mode_text != current_mode:
                    ...
                    send_telegram_message(f"🔄 *تبديل الوضع!*\n📅 {datetime.now(CAIRO_TZ).strftime('%A %d/%m/%Y')}\n🌐 الوضع الجديد: {mode_text}\n📋 الأزواج: {len(pairs)}")
                    with data_lock:
                ...
                if mg_queue_copy:
                    ...
                    if now_time - state.last_hunt_message_time >= MARTINGALE_HUNT_INTERVAL:
                        ...
                        send_telegram_message(f"🔍 *البحث عن مارتينجيل...*\n🎯 تحليل كل الأزواج المتاحة.\n⏳ البحث عن *قوية جداً* 🔵 أو أعلى.\n✅ كل الإشارات العادية شغالة.")
                active_pairs = []
                disabled_count = 0
