...
def get_regime_badge(strategy_name, regime, htf_data=None, indicator_counts=None):
    """
    بترجع جملة توضح مدى ملاءمة الاستراتيجية للسوق الحالي مع عرض المؤشرات
    """
    enhancers = ""
    if indicator_counts and isinstance(indicator_counts, dict):
        htf = indicator_counts.get("1H", {})
        ltf = indicator_counts.get("5m", {})
        enhancers = f"\n📊 [1H]: {htf.get('CALL',0)}↑ / {htf.get('PUT',0)}↓ / {htf.get('NEUTRAL',0)}!\n📊 [5m]: {ltf.get('CALL',0)}↑ / {ltf.get('PUT',0)}↓ / {ltf.get('NEUTRAL',0)}!"
    ...
    return badges.get(strategy_name, badges['original']).get(regime, "🌫️ سوق مختلط") + enhancers
...
def calculate_supertrend(df, period=10, multiplier=3):
    ...
    try:
        ...
        return pd.Series([0] * len(df)), pd.Series([1] * len(df))
...
def get_htf_market_regime(pair):
    """تحليل HTF المتقدم مع Supertrend, MACD, EMA, ALMA"""
    ...
    try:
        ...
        if adx >= 25 and atr > atr_avg * 1.2 and trend_strength == "strong":
            regime = "trending"
...
def detect_market_regime(pair, tf=300):
    ...
    try:
        ...
        if htf_confidence >= 60:
            logger.info(f"📊 Regime {pair}: LTF={ltf_regime} | HTF={confirmed_regime} | Conf={htf_confidence}%")
    ...
    if 280 <= csec <= 299:
        ...
        if csec >= 293:
            ...
            final_signal = send_final_signal(
                ...
                duration_text, indicators_str, 'original', regime=regime, htf_data=htf_data, indicator_counts=indicator_counts
            )
...
# ========== TRADE RESULTS CHECK (معدل - بدون رسائل) ==========
...
def run_bot():
    ...
    send_telegram_message(
        ...
        f"📊 *HTF Enhanced:* Supertrend + MACD + EMA + ALMA"
    )
