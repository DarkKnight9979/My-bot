...
VERSION = "7.9-FINAL-OPTIMIZED-THRESHOLDS-FIXED"
...
# ========== SIGNAL NAMES - NEW LEVELS (75-120) ==========
SIGNAL_NAMES = {
    1: ("جيدة 🟢", "GOOD"),
    2: ("قوية 🔵", "STRONG"),
...
def analyze_pair_quantum(pair, timeframe="5m"):
    ...
    # ===== تحليل حالة السوق مع خصم نقاط بدلاً من الرفض =====
    regime = analyze_market_condition_quantum(df, pair=pair)
    
    regime_penalty = 0
    if regime == "ranging":
        regime_penalty = 5  # خصم 5 نقاط بدلاً من الرفض
        logger.info(f"⚠️ Quantum {pair}: سوق عرضي (خصم 10 نقاط)")
    elif regime == "low_vol":
        regime_penalty = 3
        logger.info(f"⚠️ Quantum {pair}: تقلب منخفض (خصم 5 نقاط)")
    elif regime == "high_vol":
        regime_penalty = 3
        logger.info(f"⚠️ Quantum {pair}: تقلب عالي (خصم 3 نقاط)")
    ...
    # ===== فلتر التقلب مع خصم نقاط بدلاً من الرفض =====
    vol_filter = analyze_volatility_filter(volatility)
    vol_penalty = 0
    if not vol_filter['can_enter']:
        vol_penalty = 3  # خصم 3 نقاط بدلاً من الرفض
    ...
    # ===== FVG مع خصم نقاط بدلاً من الرفض =====
    ...
    if fvg:
        ...
        fvg_penalty = 10
        logger.info(f"⚠️ Quantum {pair}: لا يوجد FVG (خصم 10 نقاط)")
    ...
    # ===== تطبيق جميع العقوبات =====
    total_penalty = regime_penalty + fvg_penalty + vol_penalty
    adjusted_score = result['score'] - total_penalty
        ...
        with data_lock:
            if pair_key not in state.quantum_alerted_pairs:
                ...
                msg = (
                    ...
                    f"الزوج: `{pair}` [5 دقائق]\n"
                    f"الاتجاه: *{da}*\n"
                    f"📊 النقاط: *{adjusted_score}/120* (معدلة)\n"
                    f"⏱️ *صفقة قادمة خلال 20 ثانية...*\n"
                    f"🔄 *جاري التحقق من الشروط النهائية...*\n"
                    f"━━━━━━━━━━━━\n"
                    f"🕐 *الوقت:* {time_quality}\n"
    ...
    if csec >= 293:
        ...
        indicators_str = f"Score={adjusted_score}/120 | " + " | ".join(result['reasons'][:3])
        ...
        final_signal = send_final_signal(
            pair, result['direction'], signal_name_ar, adjusted_score,
...
def evaluate_signal_strength_enhanced(direction, curr, prev, df, price, alma9, alma50,
    ...
    if has_cross:
        score += w["trend"]
        reasons.append("ALMA Cross +20")
    elif trend_aligned:
        score += int(w["trend"] * 0.6)
        reasons.append("ALMA Aligned +12")
    
    if htf_regime and htf_regime.get("trend") == direction:
        score += int(w["trend"] * 0.5)
        reasons.append("HTF Confirm +10")
    
    if direction == "CALL":
        stoch_ok = stoch_k > stoch_d and stoch_k > 45
        roc_ok = roc > 0.015
    else:
        stoch_ok = stoch_k < stoch_d and stoch_k < 55
        roc_ok = roc < -0.015
    
    if stoch_ok:
        score += int(w["momentum"] * 0.7)
        reasons.append("Stoch OK +14")
    if roc_ok:
        score += int(w["momentum"] * 0.5)
        reasons.append("ROC OK +10")
    
    if structure and ((direction == "CALL" and structure == "BULLISH") or (direction == "PUT" and structure == "BEARISH")):
        score += w["structure"]
        reasons.append("Structure +15")
    
    vol_ratio = volume / vol_ma if vol_ma > 0 else 0
    if vol_ratio >= 1.5:
        score += w["volume"]
        reasons.append("Vol Strong +15")
    elif vol_ratio >= 1.0:
        score += int(w["volume"] * 0.6)
        reasons.append("Vol OK +9")
    elif vol_ratio < 0.3:
        return 0, []
    ...
    if body_pct >= 0.55 and shadow_pct <= 0.35:
        score += w["candle"]
        reasons.append("Candle Strong +15")
    elif body_pct >= 0.40 and shadow_pct <= 0.45:
        score += int(w["candle"] * 0.5)
        reasons.append("Candle OK +7")
    else:
        return 0, []
...
def analyze_pair(pair, timeframe="5m"):
    ...
    if regime == "ranging":
        regime_penalty = 10
        logger.info(f"⚠️ {pair}: Original — سوق عرضي (خصم 10 نقاط)")
    elif regime == "low_vol":
        regime_penalty = 3
    ...
    if bullish_cross and stoch_k > stoch_d and stoch_k > 45:
        potential_direction = "CALL"
    elif bearish_cross and stoch_k < stoch_d and stoch_k < 55:
        potential_direction = "PUT"
    elif price > alma9 and stoch_k > stoch_d and rsi <= 65 and stoch_k > 45:
        potential_direction = "CALL"
    elif price < alma9 and stoch_k < stoch_d and rsi >= 35 and stoch_k < 55:
        potential_direction = "PUT"
    ...
    if htf_structure:
        if (potential_direction == "CALL" and htf_structure == "BEARISH") or \
           (potential_direction == "PUT" and htf_structure == "BULLISH"):
            htf_penalty += 10
            logger.info(f"⚠️ {pair}: HTF Structure عكسي (خصم 10 نقاط إضافية)")
...
def analyze_pair_king(pair, timeframe="5m"):
    ...
    if regime == "ranging":
        ...
        regime_penalty = 3
        logger.info(f"⚠️ King {pair}: تقلب منخفض (خصم 5 نقاط)")
    ...
    if structure == "NEUTRAL":
        ...
        if adx_check < 6:  # من 12 ← 6
            logger.info(f"🛑 King {pair}: NEUTRAL و ADX={adx_check:.1f} < 6")
            return None
        else:
            logger.info(f"ℹ️ King {pair}: NEUTRAL لكن ADX={adx_check:.1f} >= 6")
    ...
    if not sweep_ok:
        if adx < 3:
            logger.info(f"🛑 King {pair}: لا يوجد Sweep و ADX={adx:.1f} < 3")
            return None
        else:
            logger.info(f"ℹ️ King {pair}: لا يوجد Sweep لكن ADX={adx:.1f} >= 3")
    ...
    # ===== العتبة ثابتة 75 =====
    if score < 45:
        logger.info(f"🛑 King {pair}: Score={score} < 45")
        return None
    ...
    if 280 <= csec <= 299:
        ...
        if csec >= 293:
            ...
            indicator_counts = get_indicator_counts(pair, df)
            indicators_str = f"Score={score}/120 | ADX={adx:.1f} | RSI={rsi:.1f}"
            htf_data = get_htf_market_regime(pair)
            final_signal = send_final_signal(
                pair, potential_direction, signal_name_ar, score,
...
def analyze_pair_smc(pair, timeframe="5m"):
    ...
    if regime == "ranging":
        regime_penalty = 10
        logger.info(f"⚠️ SMC {pair}: سوق عرضي (خصم 10 نقاط)")
    elif regime == "low_vol":
        regime_penalty = 3
    ...
    if (bias == "CALL" and structure == "BULLISH") or (bias == "PUT" and structure == "BEARISH"):
        score += 30
        conf.append("Structure")
    else:
        score += 15  # base score even if structure neutral
        conf.append("Structure(weak)")
    ...
    if (bias == "CALL" and curr['ALMA_20'] > curr['ALMA_50']) or (bias == "PUT" and curr['ALMA_20'] < curr['ALMA_50']):
        ...
        score += 8
        conf.append("Trend(weak)")

    if (bias == "CALL" and 20 <= rsi <= 60) or (bias == "PUT" and 40 <= rsi <= 80):
        score += 15
        conf.append("RSI")
    else:
        score += 8
        conf.append("RSI(weak)")

    score = score - regime_penalty

    if score < 45:
        logger.info(f"🛑 SMC {pair}: Score={score} < 45")
        return None

    # SMC: relaxed - no hard requirement for sweep/OB/breaker/FVG
    if not (sweep_ok or ob_hit or bb_hit or fvg_hit):
        logger.info(f"⚠️ SMC {pair}: لا يوجد Sweep/OB/Breaker/FVG — Score منخفض")
...
def analyze_pair_pro(pair, timeframe="5m"):
    ...
    if regime == "ranging":
        regime_penalty = 5
        logger.info(f"⚠️ Pro {pair}: سوق عرضي (خصم 5 نقاط)")
    elif regime == "low_vol":
        regime_penalty = 10
        logger.info(f"⚠️ Pro {pair}: تقلب منخفض (خصم 10 نقاط)")
    ...
    if structure == "BULLISH":
        ...
        if at_sup and lower_wick > body * 0.3 and curr['Close'] > curr['Open']:
            score = 50
            direction = "CALL"
        ...
        if at_res and upper_wick > body * 0.3 and curr['Close'] < curr['Open']:
            score = 50
            direction = "PUT"
    ...
    # ===== العتبة ثابتة 75 =====
    if direction is None or score < 45:
        logger.info(f"🛑 Pro {pair}: Score={score} < 45 أو لا يوجد اتجاه")
        return None

    # Base score boost if structure is clear
    if structure in ["BULLISH", "BEARISH"] and score < 50:
        score += 10
        logger.info(f"ℹ️ Pro {pair}: تعزيز Structure +10 نقاط")
...
def update_news():
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        ...
                        logger.info(f"✅ تم تحديث الأخبار: {len(data)} حدث من {url}")
                        return
                    else:
                        logger.warning(f"⚠️ مصدر {url} رجع بيانات فاضية")
            except requests.exceptions.Timeout:
                logger.warning(f"⏳ Timeout في {url} (محاولة {attempt+1}/3)")
                time.sleep(2 ** attempt)
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"🔌 Connection Error في {url}: {e}")
                time.sleep(2 ** attempt)
            except Exception as e:
                logger.warning(f"⚠️ خطأ في {url}: {e}")
                last_error = str(e)
                time.sleep(1)
    ...
    if state.news_data and len(state.news_data) > 0:
        logger.warning(f"⚠️ فشل تحديث الأخبار، باستخدام {len(state.news_data)} حدث قديم")
    else:
        logger.error(f"❌ فشل المصادر في جلب الأخبار: {last_error}")

def is_news_for_pair(pair):
    """التحقق من وجود أخبار عالية التأثير للزوج خلال الـ 15 دقيقة الجاية"""
    ...
    with data_lock:
        news_snapshot = list(state.news_data) if state.news_data else []
    ...
    for ev in news_snapshot:
        try:
            ...
            if impact not in ['HIGH', 'RED', '3', 'HIGH IMPACT', 'H']:
                continue
            
            curr_raw = ev.get('country') or ev.get('currency') or ''
            curr = str(curr_raw).upper().strip()
            
            if not curr or curr not in CURRENCY_PAIRS:
                continue
            
            if pair not in CURRENCY_PAIRS.get(curr, []):
                continue
            ...
            if isinstance(ev_date, (int, float)):
                et = datetime.fromtimestamp(ev_date, tz=UTC_TZ)
                ...
                for fmt in date_formats:
                    try:
                        et = datetime.strptime(ev_date, fmt).replace(tzinfo=UTC_TZ)
                        break
                    except ValueError:
                        continue
                ...
                if not et:
                    try:
                        et = pd.to_datetime(ev_date, utc=True)
                    except Exception:
                        continue
            ...
            if diff <= 900:
                title = ev.get('title', ev.get('name', 'Unknown'))
                logger.info(f"📰 خبر عالي التأثير: {title} | {curr} | {et.strftime('%H:%M')}")
                return True
...
def passes_common_entry_filters(pair):
    """التحقق من الفلاتر العامة (الأخبار وافتتاح السوق)"""
                ...
                if target_candle is None:
                    for c in reversed(candles):
                        if c.get('to', 0) < current_time:
                            target_candle = c
                            logger.warning(f"⚠️ {pair}: استخدام fallback")
                            break
