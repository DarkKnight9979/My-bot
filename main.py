...
def is_news_for_pair(pair):
    ...
    with data_lock:
        if state.news_fetch_failed:
            logger.warning("⚠️ شبكة الأخبار غير متاحة — إيقاف الدخول مؤقتاً (Fail-Safe)")
            return True
...
def has_open_trade_for_pair(pair):
    with data_lock:
        for t in state.active_trades:
            if t.get('pair') == pair:
                return True
...
def analyze_pair(pair, timeframe="5m"):
    ...
    if 295 <= csec <= 299:
        ...
        if has_open_trade_for_pair(pair):
            logger.info(f"🛑 إشارة {pair} مرفوضة (فيه صفقة مفتوحة بالفعل على نفس الزوج)")
            return None
...
def analyze_pair_king(pair, timeframe="5m"):
    ...
    if 295 <= csec <= 299:
        ...
        if has_open_trade_for_pair(pair):
            logger.info(f"🛑 King إشارة {pair} مرفوضة (فيه صفقة مفتوحة بالفعل على نفس الزوج)")
            return None
